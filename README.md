...
10. **ultra-prompt** — turns a task into a disciplined prompt for Claude Code's `ultra*code` / dynamic-workflow runs, where the difference between a tight, scoped, plan-first brief and a vague one-liner is a 10x token swing. Always returns the prompt *plus* the cheapest configuration that still clears the bar, and ships with a keyword-safety convention so the spec itself can't accidentally launch a workflow.
