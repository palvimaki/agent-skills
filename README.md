...
- **[ultra-prompt.md](ultra-prompt.md)** — writes a best-practice prompt for Claude Code's `ultra*code` / dynamic-work\*flow feature and pairs it with a tailored token-efficiency configuration (effort tier, scope, per-stage model, run hygiene). Product-specific by design — it is *about* one named feature — but runner-agnostic: any model can follow it to produce the prompt. Includes a keyword-safety convention that breaks every trigger term with an asterisk (`ultra*code`, `work*flow`) so pasting the spec into a live agent never trips it into work\*flow mode; the asterisk is stripped only in the final prompt handed to the user.
...
`ultra-prompt` is the one spec that is intentionally tied to a single product — Claude Code's `ultra*code` / dynamic-work\*flow feature — because its whole job is to write good prompts for *that* feature. It stays portable in the only sense that matters here: any model can run it, and its mechanics section tells the installing agent to re-verify version-specific details against current docs. Its keyword-safety convention (every trigger term broken with an asterisk) is deliberate, not a typo — see the spec.
...
10. **ultra-prompt** — turns a task into a disciplined prompt for Claude Code's `ultra*code` / dynamic-work\*flow runs, where the difference between a tight, scoped, plan-first brief and a vague one-liner is a 10x token swing. Always returns the prompt *plus* the cheapest configuration that still clears the bar, and ships with a keyword-safety convention so the spec itself can't accidentally launch a work\*flow.
